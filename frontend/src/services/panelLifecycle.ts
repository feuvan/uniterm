...
export function isPanelLifecycleCancelled(error: unknown): error is PanelLifecycleCancelledError {
  if (error instanceof PanelLifecycleCancelledError) return true
  if (!error || typeof error !== 'object') return false
  const candidate = error as { name?: unknown; message?: unknown }
  return candidate.name === 'PanelLifecycleCancelledError'
    && candidate.message === 'Panel lifecycle operation was cancelled'
}
...
export class PanelLifecycle {
  ...
  private disposingResources = new Set<string>()
  ...
  async createSession(
    ...
    await this.releasePrimarySession(panelId)
  ...
  async createChildSession(
    ...
    this.trackSession(panelId, info.id, false)
    try {
  ...
  async disposeOwnedSession(panelId: string, sessionId: string): Promise<void> {
    const owned = this.sessions.get(panelId)?.delete(sessionId) ?? false
    const primary = this.primarySessions.get(panelId) === sessionId
    if (primary) this.primarySessions.delete(panelId)
    ...
    if (owned || primary || bound) await this.closeQuietly(sessionId)
  ...
  async adoptSession(panelId: string, sessionId: string, generation: number): Promise<void> {
    ...
    this.trackSession(panelId, sessionId, true)
    try {
  ...
  /** Close the primary session while retaining child companions for retry. */
  disposeSession(panelId: string): Promise<void> {
    if (this.disposingResources.has(panelId)) return Promise.resolve()
    const panelDisposal = this.panelDisposals.get(panelId)
    if (panelDisposal) return panelDisposal
    if (!this.state.hasPanel(panelId)) return Promise.resolve()
    this.invalidate(panelId)
    return this.releasePrimarySession(panelId)
  }
  ...
  private releasePrimarySession(panelId: string): Promise<void> {
    const existing = this.sessionDisposals.get(panelId)
    if (existing) return existing
    const disposal = Promise.resolve().then(async () => {
      const ids = new Set<string>()
      const primaryId = this.primarySessions.get(panelId)
  ...
  private async releaseChildSessions(panelId: string): Promise<void> {
    const primaryId = this.primarySessions.get(panelId)
    const ids = [...(this.sessions.get(panelId) ?? [])]
      .filter(sessionId => sessionId !== primaryId)
  ...
  private async disposePanelInternal(panelId: string): Promise<void> {
    ...
    // Reentrant disposal calls from a resource must not await the outer teardown.
    this.disposingResources.add(panelId)
    try {
      for (const dispose of [...(panelResources ?? [])].reverse()) {
        try { await dispose() } catch { /* release the rest even if one fails */ }
      }
    } finally {
      this.disposingResources.delete(panelId)
    }
    await this.releaseChildSessions(panelId)
    await this.releasePrimarySession(panelId)
    this.state.removePanel(panelId)
  }

  private trackSession(panelId: string, sessionId: string, primary: boolean): void {
    ...
    ids.add(sessionId)
    if (primary) this.primarySessions.set(panelId, sessionId)
  }
