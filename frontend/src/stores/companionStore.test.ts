...
beforeEach(() => {
  ...
  panelId = usePanelStore().createPanel({
    ...
    user: 'u', password: 'test', authType: 'password',
  }, 'ssh').id
})
...
describe('companion session ownership', () => {
  it.each(['ensureSftp', 'ensureMonitor'] as const)('%s shares one pending create between callers', async (method) => {
    ...
    expect(api.create).toHaveBeenCalledWith(method === 'ensureSftp' ? 'sftp' : 'monitor', expect.objectContaining({ initialCols: 0, initialRows: 0 }))
  })
